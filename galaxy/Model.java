...
class Model {
     ...
     */
    static final ArrayList<Place> EMPTYLIST = new ArrayList<>();
    /**
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (isCenter(boundary) && !(boundary.x == center.x
                        && boundary.y == center.y)) {
                    return false;
                }
            }
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (isCell(center)) {
            ...
            galaxy.add(center.move(0, -1));
        } else if (isVert(center)) {
            galaxy.add(center.move(-1, 0));
        } else if (isIntersection(center)
                && !isBoundary(center.move(1, 0))
                && !isBoundary(center.move(-1, 0))
                && !isBoundary(center.move(0, 1))
                && !isBoundary(center.move(0, -1))) {
            galaxy.add(center.move(-1, -1));
        }
        HashSet<Place> secondHash = new HashSet<>();
        for (Place c : galaxy) {
            accreteRegion(c, secondHash);
        }
        galaxy.addAll(secondHash);
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        markAll(region, 1);
        List<Place> convertRegion = new ArrayList<>(region);
        while (!unmarkedSymAdjacent(center, convertRegion).equals(EMPTYLIST)) {
            convertRegion.addAll(unmarkedSymAdjacent(center, convertRegion));
            markAll(convertRegion, 1);
        }
        region.addAll(convertRegion);
        markAll(region, 0);
        return region;
    }
    ...
    List<Place> unmarkedContaining(Place place) {
        if (isCell(place)) {
            ...
            if (mark(place.x - 1, place.y) == 0
                    && mark(place.x + 1, place.y) == 0) {
                return asList(place.move(-1, 0), place.move(1, 0));
            }
        } else if (isHoriz(place)) {
            if (mark(place.x, place.y - 1) == 0
                    && mark(place.x, place.y - 1) == 0) {
                return asList(place.move(0, -1), place.move(0, 1));
            }
        } else {
            Place[] placelist = new Place[4];
            for (int i = 0; i < 4; i += 1) {
                int dx = 2 * (i / 2) - 1;
                int dy = 2 * (i % 2) - 1;
                ...
                placelist[i] = place.move(dx, dy);
            }
            return asList(placelist[0], placelist[1],
                    placelist[2], placelist[3]);
        }
        return Collections.emptyList();
    }
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ...
        for (Place r : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (opp != null && mark(opp) == 0 && mark(p) == 0) {
                    result.add(p);
                }
            }
